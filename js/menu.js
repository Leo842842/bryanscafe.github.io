...
fetchMenu() ? populateMenu(mealList, hotBevs, otherBevs): fetchMenu(); // fetchMenu()
};
...
hotBevs = XHR.responseXML.getElementsByTagName('hotBeverages')[0].children; otherBevs = XHR.responseXML.getElementsByTagName('otherBeverages')[0].children; return true
} else return false;
}
function generateCard (name, price, desc, imageURL) {
    let card = document.createElement("div");
    card.classList.add("col"); card.innerHTML =`
    <div class="card">
    <img src="${imageURL}" class="card-img-top" alt="Menu item ${name}"> <div class="card-body">
    <h5 class="card-title">${name}</h5>
    <p class="card-text">${desc}</p>
    <p class="card-text">$${price}</p> </div>
    </div>`;
    return card;
}
...
function populateBeverages (hotBevs, otherBevs) {
    ...
    for (let i = 0, n = hotBevs.length; i < n; i++) {
        let size = `Coffee and hot chocolates - ${hotBevs[i].getElementsByTagName("size")[0].textContent}`; let price = hotBevs[i].getElementsByTagName("price")[0].textContent;
    ...
    let name = otherBevs[i].getElementsByTagName("name")[0].textContent; let price = otherBevs[i].getElementsByTagName("price")[0].textContent; let imageURL = "images/beverage.jpg";
    let desc = "";
    menu.appendChild(generateCard(name, price, desc, imageURL));
    }
}